// dist：0 在顶面外轮廓边线，向内增大 → 渐变与高光沿轮廓，而非主 UV 矩形（模式 0 仅作兼容）
// 模式 1：UV1.x 烘焙「到顶面外轮廓」距离；模式 2：_ContourTex；模式 3：物体空间圆柱 XZ；模式 4：物体空间矩形 XZ（与轴对齐方块顶面一致）
Shader "Custom/old_Hologramlv"
{
    Properties
    {
        ...
        _ContourMode ("轮廓模式 0主UV矩形 1UV1 2贴图 3圆柱XZ 4矩形XZ", Range(0, 4)) = 0
